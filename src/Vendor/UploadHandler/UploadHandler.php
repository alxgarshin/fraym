...
namespace Fraym\Vendor\UploadHandler;
...
class UploadHandler
{
    ...
    protected array $error_messages = [
        1 => 'Загруженный файл превышает допустимый размер!',
        2 => 'Загруженный файл превышает допустимый размер.',
        3 => 'Файл был загружен только частично!',
        4 => 'Не определен файл для загрузки!',
        6 => 'Отсутствует временная папка!',
        7 => 'Ошибка записи файла на диск!',
        8 => 'Расширение PHP остановило загрузку файла!',
        'post_max_size' => 'Загруженный файл превышает допустимый размер для размещения!',
        'max_file_size' => 'Файл слишком большой!',
        'min_file_size' => 'Файл слишком маленький!',
        'accept_file_types' => 'Файлы с таким расширением загружать нельзя!',
        'max_number_of_files' => 'Максимальное количество файлов превышено!',
        'max_width' => 'Ширина изображения превышает допустимую!',
        'min_width' => 'Ширина изображения меньше допустимой!',
        'max_height' => 'Высота изображения превышает допустимую!',
        'min_height' => 'Высота изображения меньше допустимой!',
    ];
    ...
    {
        $LOCALE_GLOBAL = LocaleHelper::getLocale(['fraym']);

        $this->error_messages = $LOCALE_GLOBAL['classes']['UploadHandler']['error_messages'];

        $this->options = [
            'script_url' => $this->get_full_url(),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . $_ENV['UPLOADS_PATH'],
            'upload_url' => $this->get_full_url() . $_ENV['UPLOADS_PATH'],
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'uploads',
    ...
    protected function get_full_url()
    {
        ...
        return ABSOLUTE_PATH . '/' . mb_substr(
            $_SERVER['SCRIPT_NAME'],
            1,
            mb_strrpos($_SERVER['SCRIPT_NAME'], '/'),
        );
    }

    protected function get_user_id(): void
    {
        return;
    }
    ...
    protected function set_additional_file_properties($file): void
    {
        $file->deleteUrl = $this->options['script_url']
            ...
            . '=' . rawurlencode($file->name) . '&type=' . $GLOBALS["uploadType"];
    ...
    {
        return ($this->error_messages[$error] ?? false) ?
    ...
    protected function validate($uploaded_file, $file, $error)
    {
        ...
        if (
            is_int($this->options['max_number_of_files']) && ($this->count_file_objects() >= $this->options['max_number_of_files'])
        ) {
    ...
    protected function handle_form_data($file, $index): void
    {
    ...
    protected function gd_set_image_object($file_path, $image): void
    {
        $this->gd_destroy_image_object($file_path);
    ...
    protected function gd_create_scaled_image($file_name, $version, $options)
    {
        ...
        $type = mb_strtolower(mb_substr(mb_strrchr($file_name, '.'), 1));
    ...
    protected function imagick_set_image_object($file_path, $image): void
    {
        $this->imagick_destroy_image_object($file_path);
        $this->image_objects[$file_path] = $image;
    }
    ...
    protected function header($str): bool
    {
        header($str);
    ...
    {
        //return mb_substr($this->options['param_name'], 0, -1);
    ...
    {
        $params = $_GET[$this->options['param_name']] ?? [];
